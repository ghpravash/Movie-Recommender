...
def recommend(request):
    ...
    movie_list = list(Movie.objects.filter(
        id__in=movies_id_recommend).order_by(preserved)[:k])
    mymovies = (list(Movie.objects.filter()[:random.randint(1, 48)]))
    random.shuffle(mymovies)
    context = {'movie_list': mymovies[:random.randint(1,k)]}
    return render(request, 'recommend/recommend.html', context)

    df = pd.DataFrame(list( Movie.objects.filter()[:random.randint(0, 5)]))
    ...
    real_neighbors = list(
        sorted(real_neighbors, key=lambda x: x[1], reverse=True))
    real = []
    for i in real_neighbors:
        real.append(i[0])
    context = {'movie_list': list(
        Movie.objects.filter()[:random.randint(0, 5)])}
    return render(request, 'recommend/recommend.html', context)
